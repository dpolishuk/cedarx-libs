...
video_decoder_t* g_vdecoder = NULL;

void libcedarv_free_vbs_buffer_sem(void)
{
	if(g_vdecoder && g_vdecoder->icedarv.free_vbs_buffer_sem)
		g_vdecoder->icedarv.free_vbs_buffer_sem(g_vdecoder->icedarv.cedarx_cookie);
}
...
static s32 vdecoder_io_ctrl(cedarv_decoder_t* p, u32 cmd, u32 param)
{
    ...
    switch(cmd)
    {
        ...
        case CEDARV_COMMAND_JUMP:
        {
            ...
            if(decoder->fbm != NULL)
            {
                if(decoder->display_already_begin)
                    fbm_reset(decoder->last_frame_index, decoder->fbm);
                else
                    fbm_reset(0xff, decoder->fbm);
...
static void vdecoder_set_vbv_vedec_memory(video_decoder_t* p)
{
    ...
    {
        p->max_vbv_buffer_size  = 8*1024*1024;
        p->config_info.max_memory_available = 0;							//* tell libvecore don't care memory usage.
        p->config_info.max_output_width     = MAX_SUPPORTED_VIDEO_WIDTH;	//* max output video width, if exceed, video will be scale down to fit;
        p->config_info.max_output_height    = MAX_SUPPORTED_VIDEO_HEIGHT;	//* max output video height, if exceed, video will be scale down to fit;
    }
    else if(p->remainMemorySize >= 32*1024*1024)
    {
        p->max_vbv_buffer_size = 8*1024*1024;
        p->config_info.max_memory_available = p->remainMemorySize - p->max_vbv_buffer_size;

        //* set maximum output size to be 1440*800, for source of size 1920x1080, video will be scale down to fit.
        //* this method can save some memory space because picture frame buffer is smaller.
        p->config_info.max_output_width     = 1440;
        p->config_info.max_output_height    = 800;

        if(p->stream_info.format == STREAM_FORMAT_H264)
        {
            p->config_info.scale_down_enable = 1;	//* for H264, we should set this flag to tell that decoder can scale the output pictures.
        }
    }
    else if(p->remainMemorySize >= 16*1024*1024)
    {
        p->max_vbv_buffer_size = 6*1024*1024;
        p->config_info.max_memory_available = p->remainMemorySize - p->max_vbv_buffer_size;
        if(p->stream_info.format == STREAM_FORMAT_H264)
        {
            p->config_info.scale_down_enable = 1;
            //* set maximum output size to be 1440*800, for source of size 1920x1080, video will be scale down to fit.
            //* this method can save some memory space because picture frame buffer is smaller.
            p->config_info.max_output_width  = 1440;
            p->config_info.max_output_height = 800;
